...
class ChipGroupGender {
    ...
    sealed class ListType {
        ...
        class ChildList(val listOfChildHobbies: List<GenderObject>) : ListType()
    }
        ...
        fun build(): Builder {
            ...
            llInterests?.let { thisView ->
                ...
                txtSave?.setOnClickListener {
                                        ...
                                        (listType as ListType.ChildList)
                                            .listOfChildHobbies.forEach {
                                                ...
                                                if (it.title == listTop!![i]) {
                                                    ...
                                                    Log.d(TAG, "build321231asdsa: " + it.id)
                    ...
                    Log.d(TAG, "buildsd: " + selectedId1)
                    Constant.savbtnClick.value = selectedId1
        ...
        private fun buildChip(name: String): Chip? {
            ...
            if (isCloseIconVisible && !isCurrentBuilderChild) {
                ...
                /*
                                chip?.setOnClickListener { vie ->
                                  //  chip?.isChecked=false
                                    var b = (vie as Chip).isChecked
                                    if (b) {

                                        Log.d(TAG, "selectedItem734" + "firstCall")
                                        selectedCount += 1
                                        //selectedCount -= 1
                ...
                /*  for (ii in selectedItemsList.indices) {
                                              Log.d(
                                                  TAG,
                                                  "buildChdfdfip: " + selectedItemsList[ii] + "--->" + vie.text.toString() + "--->" + selectedCount
                                              )
                                              if (selectedItemsList[ii] != vie.text.toString()) {
                                                  Log.d(TAG, "buildChdfdfip: " + ii)
                                                  selectedCount -= 1
                                                  selectedItemsList.removeAt(selectedCount)
                                                  //   selectedItemsList.remove(abc)
                                                  break
                                              }
                                          }*//*

                        Log.d(TAG, "selectedItem734" + selectedCount + "-->" + maxSelectedCount)

*/
                /*
                                        if (selectedCount == maxSelectedCount) {
                                            for (ii in selectedItemsList.indices) {
                                                Log.d(TAG, "selectedItem:761 " + maxSelectedCount)
                                                selectedCount = (maxSelectedCount - 1)
                                                Log.d(TAG, "selectedItem:743 " + selectedItemsList[ii])
                                                Log.d(TAG, "selectedItem:744 " + vie.text.toString())
                                                if (selectedItemsList[ii] != vie.text.toString()) {
                                                    Log.d(TAG, "selectedItem:734 " + ii)

                                                    selectedItemsList.removeAt(ii)
                                                    selectedItemsList.removeAt((selectedCount - 1))
                                                    vie.isChecked = true
                                                    checkChanging = true
                                                    break
                                                }
                ...
                *//*
                        ...
                        Log.d(TAG, "selectedItem734 " + "second Call")
                        selectedCount -= 1
                        for (ii in selectedItemsList.indices) {
                            selectedCount = (maxSelectedCount - 1)
                            if (selectedItemsList[ii] == vie.text.toString()) {
                    ...
                    Log.d(TAG, "buildChip3212: " + selectedCount + "-->" + maxSelectedCount)

                    // try {
                    */
                /*if (selectedCount == maxSelectedCount) {
                                        Log.d(TAG, "selectedItem:750 " + vie.text.toString())
                                        Log.d(TAG, "selectedItem:751 " + maxSelectedCount)
                                        selectedCount = (maxSelectedCount - 1)
                                        ...
                                        Log.d(TAG, "selectedItem:756 " + vie)
                    ...
                    if (selectedCount == maxSelectedCount) {
                        ...
                        for (ii in selectedItemsList.indices) {
                            selectedCount = (maxSelectedCount - 1)
                            Log.d(TAG, "sdfdfssdf:779 " + ii)
                            if (selectedItemsList[ii] == vie.text.toString()) {
                                selectedItemsList.add(vie.text.toString())

                                Log.d(TAG, "sdfdfssdf:783 " + ii)
                                Log.d(TAG, "sdfdfssdf:7884 " + vie.text.toString())
                                selectedItemsList.removeAt(ii)
                                selectedItemsList.removeAt((selectedCount - 1))
                                vie.isChecked = true
                                checkChanging = true
                        ...
                        Log.d(TAG, "selectedItem:756 " + vie.text.toString())

                    }

                    Log.d(TAG, "selectedItem734" + "foure")
